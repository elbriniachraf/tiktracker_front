...
@Component({
  ...
  styleUrl: './gestion-contacts.component.css',
  providers: [ContactService], // Service injecté

})
export class GestionContactsComponent {
  ...
  public autoCompleteConfig = {
    ...
    addFournisseur(): void {
      ...
      this.contactService.addContact(fournisseur).subscribe({
        next: (response: any) => {
        ...
        error: (error: any) => {
          ...
          alert('Une erreur s\'est produite lors de l\'ajout du fournisseur.');
        }
      });
    }
      ...
      if (this.selectedRow) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.contactService.deleteContact(this.selectedRow._id).subscribe({
              next: (response: any) => {
    ...
    loadContacts(): void {
      ...
      this.contactService.getContacts(this.currentPage, this.pageSize,this.typeFilter,this.query).subscribe(
